...
module.exports = function(io){
	try{
		...
		io.sockets.on('connection', function(connection){
			...
			socket.on('getEnvironmentRecords', function(conf){
				log.debug(conf);
				
				environment.get(conf, function(err, result){
					log.trace('get Environement Records' + JSON.stringify(result));
					socket.emit('setEnvironmentRecords', result);
				});
			});
		});		
	}catch(e){
